...
# 使用PCA降维并可视化
def plot_pca(X, y_true, y_pred, classes):
    ...
    for i, class_name in enumerate(classes):
        ...
        plt.scatter(X_pca[idx, 0], X_pca[idx, 1], label=f'True {class_name}', alpha=0.5)

        idx_pred = np.where(y_pred[:, i] == np.max(y_pred, axis=1))[0]
        plt.scatter(X_pca[idx_pred, 0], X_pca[idx_pred, 1], label=f'Pred {class_name}', marker='x')
...
plot_pca(x_test.reshape(len(x_test), -1), y_test, y_pred, classes)